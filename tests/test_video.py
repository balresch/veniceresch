...
async def test_quote(mock_api, async_client):
    mock_api.post("/video/quote").respond(200, json={"price_usd": 0.05})
...
async def test_transcribe(mock_api, async_client):
    route = mock_api.post("/video/transcriptions").respond(200, json={"text": "hello"})
    await async_client.video.transcribe(url="https://youtube.com/watch?v=x", response_format="json")
    body = json.loads(route.calls.last.request.content)
    assert body == {
